...
public class ImageProgram implements Iterable<LineProgram> {
	...
	public void set(int index, LineProgram newProgram) {
		if (get(index).compute().hashCode() != newProgram.compute().hashCode()) {
			throw new IllegalArgumentException("Cannot replace a program entry with one that does not result in the same data");
		}
	...
	public void optimize() {
		...
		for (int i = 1; i < lines.size(); i++) {
			...
			LineProgram lowestCostProgram = computeLowestCostProgram(thisLine, lastLine, lastLineIndex);
			...
			LineProgram lowestCostProgramForPreviousLine = computeLowestCostProgram(thisLine, previousLine, i - 1);
			
			if (lowestCostProgramForPreviousLine.cost() + lines.get(i - 1).fullCost() < lines.get(i - 1).cost() + lowestCostProgram.cost()) {
				// We can save some bytes by making the previous program a full one and using something relative to that
				lastLineIndex = i - 1;
				set(i - 1, new FullLineProgram(this, previousLine));
		...
		int finalCost = cost();
		double pct = Math.round(((initialCost - finalCost) / (double)initialCost * 10000)) / (double)100;
	...
	public byte[] save() {
		...
		for (LineProgram lineProgram : lines) {
			byteString = byteString.concat(ByteString.copyFrom(IntListPacker.pack(lineProgram.encode())));
		}
		
		return byteString.toByteArray();
	}
	
	private LineProgram computeLowestCostProgram(LineSpec thisLine, LineSpec lastLine, int lastLineIndex) {
		...
		DiffLineProgram diffLineProgram = new DiffLineProgram(this, lastLineIndex, diff);
